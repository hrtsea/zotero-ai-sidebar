...
- [3. 功能手册](#3-功能手册)
  ...
  - [3.14 PDF 公式修复缓存](#314-pdf-公式修复缓存)
- [故障排查](#故障排查)
- [相关文档](#相关文档)
...
- **`📄 原文` toggle**（默认开）：把整篇论文固定在每轮上下文里，模型不用反复取。需要只看选区时关掉，单次升级到全文用 `+ 本轮原文`。
- **`LaTeX 源` 徽章**：标题旁出现就说明这篇论文在用 arXiv LaTeX 源读，公式精确。详见 [§3.13](#313-arxiv-latex-源模式)。
- **`调试` toggle**：开启后每轮发模型的 `[Paper full text]` 块会同时落盘到 Zotero 数据目录的文件里，方便核对模型实际看到什么。文件路径会出现在 Debug Markdown 导出的末尾。
- **回答里的表格** —— 模型产出的 Markdown 管道表格在对话里直接渲染成真正的表格。
- **历史 prompt 回看** —— 输入框为空时按 `↑` / `↓` 调出之前的提问。
- **流式输出粘底** —— 自动滚到底；如果你手动向上滚了，新内容到达时**保留**你的位置不强行回弹。
- **Composer 草稿** 在 sidebar 重渲染、流式输出、tool 调用、选区变化、预设切换中都不会丢。
...
**arXiv 插图同理**——模型通过 `arxiv_get_figure` 拉取的插图会出现在对话里，并且在后续轮持续作为多模态输入挂着（你接着问"那张图右下角是什么"时，vision 模型仍然能看到这张图）。
...
- **工具栏的 `调试` toggle** 开启时，每轮发模型的 `[Paper full text]` 块会同时落到 Zotero 数据目录下的文件里——怀疑模型没看到你以为它看到的东西时用。文件路径出现在 Debug Markdown 导出的末尾。

### 3.13 arXiv LaTeX 源模式

对 arXiv 论文，插件会下载源码 `.tar.gz`、清洗 TeX，把源码而不是 PDF 文本层喂给模型。

**判断方式**：论文标题旁出现 `LaTeX 源` 徽章 = 当前在源码模式。

**带来的好处：**
- 公式以原始 LaTeX 抵达模型，不再是 PDF 文本层里 `f l θ` 这种被压扁的碎片。
- 按编号引用都能精确命中——"Eq. (3)"、"Figure 2"、"Table 1" 都映射到正确的对象。
- 模型先拿到章节目录，按需取章节正文，长论文不会一次撑爆 token 预算。

**回退路径**：没有 arXiv ID、源码下载失败、作者未公开源码——插件静默回退到 PDF 流程，用一样的 `zotero_*` 工具。

**缓存位置**：清洗后的源码以论文为单位存在 Zotero 数据目录下，Zotero 重启后仍然有效。

### 3.14 PDF 公式修复缓存

对非 arXiv 的 PDF，如果公式被 PDF 文本层压扁（双栏排版常见），插件会逐论文构建一份修复后的 markdown：

1. 首次需要读 PDF 文本的那一轮，自动识别被压扁的公式片段。
2. 每个公式区域被渲染、裁切、由视觉模型转写回 LaTeX。
3. 修复后的 markdown 以论文为单位保存，后续每一轮直接复用。

**你会感觉到：**
- **第一次**问数学公式多的论文会慢一点（trace 里能看到转写步骤）。后续问答恢复正常速度。
- 整个缓存只对每篇论文跑一次视觉转写。

用 `调试` toggle 可以看到模型实际拿到的修复后文本。
...
正常行为：
- **arXiv 论文**：首次相关提问时正在下载源码 `.tar.gz`，后续问答用缓存。
- **非 arXiv 数学多的论文**：PDF 公式修复在跑（渲染裁切 + 视觉转写）。整篇论文只跑一次。
